...
- comprehending and internalizing the concept of `usize` in Rust
    - `usize` is a type whose _**width** matches the pointer width_ of the target (32-bit on a 32-bit target, 64-bit on
      64-bit) system, and it's guaranteed big enough to hold the size of any signle object or index in that process's
      memory space. Its fundamentally about **addressing memory in this process**.
    - 